...
def process_deals_data(file_path):
    ...
    df.loc[mask, 'latitude'] = df.loc[mask, 'city'].map(lambda x: city_coords.get(x, [None, None])[0])
    df.loc[mask, 'longitude'] = df.loc[mask, 'city'].map(lambda x: city_coords.get(x, [None, None])[1])
...
def create_relational_map(deals_df, dealers_df):
    ...
    # Draw deal connections (dotted lines)
    for _, deal in deals_df.iterrows():
        user_lat, user_lon = deal['latitude'], deal['longitude']
        # Match using dealerinfo.coname
        dealer_match = dealers_df[dealers_df['coname'] == deal.get('dealerinfo.coname', '')]
        # Fallback to dealer_id if coname doesn't match
        if dealer_match.empty and 'dealerinfo.dealer_id' in deals_df.columns and '_id' in dealers_df.columns:
            dealer_match = dealers_df[dealers_df['_id'] == deal['dealerinfo.dealer_id']]
        if not dealer_match.empty:
            dealer = dealer_match.iloc[0]
            folium.PolyLine(
                locations=[[user_lat, user_lon], [dealer['latitude'], dealer['longitude']]],
                color="gray",
                weight=2,
                opacity=0.8,
                dash_array="5, 5"
...
def create_graphs(deals_df, dealers_df):
    ...
    # Relational data: Match using dealerinfo.coname
    if 'dealerinfo.coname' in deals_df.columns and 'coname' in dealers_df.columns:
        relational_df = deals_df.merge(dealers_df[['coname', 'pincode']], left_on='dealerinfo.coname', right_on='coname', how='left')
        relational_df = relational_df.groupby(['user_id', 'coname']).size().reset_index(name='deal_count')
    # Fallback to dealer_id
    elif 'dealerinfo.dealer_id' in deals_df.columns and '_id' in dealers_df.columns:
        relational_df = deals_df.merge(dealers_df[['_id', 'pincode']], left_on='dealerinfo.dealer_id', right_on='_id', how='left')
        relational_df = relational_df.groupby(['user_id', '_id']).size().reset_index(name='deal_count')
    else:
        print("No matching dealer identifier found. Deals columns:", deals_df.columns.tolist())
        print("Dealers columns:", dealers_df.columns.tolist())
        relational_df = pd.DataFrame(columns=['user_id', 'dealer', 'deal_count'])
    ...
    fig5 = px.scatter(relational_df, x='user_id', y='coname' if 'coname' in relational_df.columns else '_id', 
                      size='deal_count', color='deal_count',
                      title="User-Dealer Deal Connections", template='plotly_dark',
                      hover_data=['user_id', relational_df.columns[1], 'deal_count'])
...
def dashboard():
    if request.method == 'POST':
        ...
        if deals_file and dealers_file:
            ...
            if deals_df.empty or dealers_df.empty:
                return f"Error: No valid location data found. Dealers: {len(dealers_df)}, Users: {len(deals_df)}"
            ...
            graph1, graph2, graph3, graph4, graph5 = create_graphs(deals_df, dealers_df)
            ...
            return render_template_string(html_template, users_map=users_map, dealers_map=dealers_map, 
                                        relation_map=relation_map, graph1=graph1, graph2=graph2, 
