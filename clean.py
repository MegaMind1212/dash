...
def get_next_set_number():
    """Get the next available set number for uploads."""
    set_folders = [f for f in os.listdir(app.config['UPLOAD_FOLDER']) if f.startswith('set_')]
    if not set_folders:
        return 1
    max_set = max(int(f.split('_')[1]) for f in set_folders)
    return max_set + 1
...
def process_users_data(file_path):
    ...
    print(f"Rows with missing coords: {df[mask].shape[0]} out of {len(df)}")
    return df
...
def create_new_users_map(users_df):
    ...
    print(f"Total unique pincodes mapped: {total_pincode_count} out of {len(new_users_df['pincode'].unique())} in new users data")
...
def create_graphs(deals_df, dealers_df, users_df):
    ...
    fig1.update_layout(
        ...
        xaxis={'tickangle': 45, 'tickmode': 'auto', 'automargin': True},
...
def dashboard():
    ...
    <head>
        ...
        <style>
            ...
            .container { display: flex; flex-wrap: wrap; justify-content: center; gap: 15px; padding: 20px; margin-left: 0; max-width: 1400px; margin-right: auto; transition: margin-left 0.3s ease; }
            .container.sidebar-visible { margin-left: 220px; }
    ...
    <body>
        <div class="header">
            ...
            <div style="font-size: 12px; color: red; font-style: italic; font-weight: 300; margin-top: 5px;">
    (Under Testing)
</div>
            <div class="controls">
        ...
        <script>
            {% if current_analysis %}
                var graph1 = {{ current_analysis.graph1|safe }};
                var graph2 = {{ current_analysis.graph2|safe }};
                var graph3 = {{ current_analysis.graph3|safe }};
                var graph4 = {{ current_analysis.graph4|safe }};
                var graph5 = {{ current_analysis.graph5|safe }};
